(function () {
  ...
  function retryState() {
    try {
      return JSON.parse(localStorage.getItem("expense_sync_retry") || "{}");
    } catch {
      return {};
    }
  }
  ...
  async function pushLocalChanges() {
    const rows = (await readAll({ includeDeleted: true })).filter(
      (row) => row.id && row.synced === false,
    );
  ...
  async function pullRemoteChanges() {
    ...
    const { data, error } = await client
      .from(TABLE)
      .select(
        "id,user_id,date,category,amount,description,created_at,updated_at,deleted",
      )
